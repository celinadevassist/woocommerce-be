...
export class SearchAnalyticsController {
  ...
  @ApiOperation({
    ...
    description:
      'Returns analytics data about search queries made by clients. Includes search terms, frequency, result counts, and timestamps. Admin access required.',
  })
  @ApiParam({
    name: 'lang',
  ...
  @ApiQuery({
    ...
    required: false,
    description:
      'Filter by endpoint type (e.g., projects, image-prompts, sessions, articles, questions)',
  })
  @ApiQuery({
    name: 'startDate',
    required: false,
  ...
  @ApiQuery({
    ...
    required: false,
    type: Boolean,
    description: 'Group results by search term',
    example: false,
  })
  @ApiResponse({
    status: 200,
  ...
  async getSearchAnalytics(
    ...
    if (user?.role !== 'admin') {
      throw new ForbiddenException(
        'Only administrators can access search analytics',
      );
    }
