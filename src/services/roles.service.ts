...
export class RoleService {
  ...
  async update(
    ...
    // }
    const nameExist = roleData.name
      ? await this.roleModel.findOne({ name: roleData.name })
      : undefined;
  ...
  async get(
    filters: QueryRoleDTO,
    lang,
  ): Promise<{ pagination: IPagination; records: IRole[] }> {
    let skip;
    const pipeLine: any = [{ $match: this.queryMaker(filters) }];
    ...
    const count: number = await this.roleModel
      .find(this.queryMaker(filters))
      .countDocuments();
