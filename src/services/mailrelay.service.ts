...
export class MailrelayService {
  ...
  constructor(private configService: ConfigService) {
    this.apiUrl = this.configService.get<string>(
      'MAILRELAY_API_URL',
      'https://example.ipzmarketing.com/api/v1/send_emails',
    );
    ...
    this.logger.log(
      `   Default From: ${this.defaultFrom} (${this.defaultFromName})`,
    );
    this.logger.log(`   Templates Path: ${this.templatesPath}`);

    if (!this.authToken) {
      this.logger.warn(
        '❌ Mailrelay auth token not configured. Email sending will fail.',
      );
    } else {
      this.logger.log(
        `   Auth Token: ${this.authToken.substring(0, 10)}... (${
  ...
  async sendMail(options: EmailOptions): Promise<boolean> {
    try {
      ...
      this.logger.log(
        `   Template: ${
          options.template || 'No template (using HTML/Text directly)'
        }`,
      );
      ...
      if (options.template && !html) {
        ...
        this.logger.log(
          `   Context Keys: ${
            options.context ? Object.keys(options.context).join(', ') : 'none'
          }`,
        );

        html = await this.renderTemplate(options.template, options.context);
        this.logger.log(
          `   ✅ Template rendered successfully (${(html.length / 1024).toFixed(
            2,
          )} KB)`,
        );
        ...
        if (!text) {
          ...
          this.logger.log(
            `   📄 Text version generated (${(text.length / 1024).toFixed(
              2,
            )} KB)`,
          );
        }
      ...
      this.logger.log(
        `   HTML Size: ${
          html ? (html.length / 1024).toFixed(2) + ' KB' : 'none'
        }`,
      );
      this.logger.log(
        `   Text Size: ${
          text ? (text.length / 1024).toFixed(2) + ' KB' : 'none'
        }`,
      );
      ...
      this.logger.log(
        `   Auth Token: ${
          this.authToken ? this.authToken.substring(0, 10) + '...' : 'NOT SET'
        }`,
      );
      this.logger.log(
        `   Payload size: ${payloadSizeKB} KB (${payloadSizeBytes} bytes)`,
      );
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        this.logger.log(
          `   Message ID: ${
            response.data.message_id || response.data.id || 'not provided'
          }`,
        );
        this.logger.log('='.repeat(60));
        ...
        this.logger.error(
          `   Full error response: ${JSON.stringify(response.data, null, 2)}`,
        );
        if (response.data.errors) {
          this.logger.error('   Field errors:');
          Object.keys(response.data.errors).forEach((field) => {
            this.logger.error(
              `     - ${field}: ${response.data.errors[field].join(', ')}`,
            );
          });
        }
      ...
      if (error.response) {
        ...
        if (error.response.status === 401) {
          this.logger.error(
            '   🔒 AUTHENTICATION ERROR: Check your MAILRELAY_AUTH_TOKEN',
          );
        } else if (error.response.status === 422) {
          this.logger.error(
            '   📝 VALIDATION ERROR: Check email addresses and payload format',
          );
        } else if (error.response.status === 500) {
          this.logger.error(
            '   🔥 SERVER ERROR: Mailrelay API is having issues',
          );
        }
        ...
        this.logger.log(
          `   Available files: ${
            fs.existsSync(this.templatesPath)
              ? fs.readdirSync(this.templatesPath).join(', ')
              : 'Directory not found'
          }`,
        );
  ...
  private async prepareAttachments(attachments?: any[]): Promise<any[]> {
    ...
    for (const attachment of attachments) {
      try {
        ...
        if (attachment.path) {
          ...
          this.logger.debug(
            `       Base64 encoded size: ${(content.length / 1024).toFixed(
              2,
            )} KB`,
          );
          ...
          this.logger.warn(
            `       ⚠️ No content or path for attachment: ${attachmentName}`,
          );
        ...
        this.logger.debug(
          `       ✅ Attachment prepared: ${attachmentName} (${preparedAttachment.content_type})`,
        );
      } catch (error) {
    ...
    this.logger.log(
      `   ✅ Prepared ${preparedAttachments.length} attachment(s)`,
    );
  ...
  async sendInvoiceEmail(email: string, invoiceData: any): Promise<boolean> {
    ...
    this.logger.log(
      `   💰 Total Amount: ${invoiceData.currency} ${invoiceData.total}`,
    );
    this.logger.log(
      `   📎 Attachments: ${invoiceData.attachments?.length || 0} file(s)`,
    );
    ...
    if (invoiceData.attachments?.length > 0) {
      invoiceData.attachments.forEach((att, index) => {
        this.logger.log(
          `     Attachment ${index + 1}: ${
            att.filename || att.file_name || 'unknown'
          } at ${att.path || 'no path'}`,
        );
        if (att.path && !require('fs').existsSync(att.path)) {
          this.logger.error(
            `     ⚠️ WARNING: Attachment file does not exist: ${att.path}`,
          );
        }
      });
    }
    ...
    this.logger.log(
      `   ✉️ [INVOICE EMAIL] Result: ${result ? '✅ SUCCESS' : '❌ FAILED'}`,
    );
    return result;
  }
